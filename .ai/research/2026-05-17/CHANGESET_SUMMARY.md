...
- `README.md` and `app/README.md` - repaired v6.5.9 documentation drift for DoH fail-closed pinning, 405 tracker SDK signatures, ipapi.co GeoIP fallback, Kotlin 2.3, Android SDK 36 compile / targetSdk 35, and Room v1-v15 migration wording. Public docs now also surface the unresolved root/app license-file conflict instead of asserting one canonical license.
...
- Argon2id KDF migration - added Bouncy Castle `bcprov-jdk18on`, introduced shared `PasswordKdf`, changed new parental PIN hashes and encrypted backup exports to Argon2id, kept PBKDF2 verification/import for legacy PIN and backup records, and added focused KDF/backup regression coverage.
- AES-GCM nonce guard - added `BackupNonceLedger`, a bounded export-time key-id/IV tuple ledger for v2 backups, wired it into `BackupCrypto.encrypt`, and covered duplicate tuple rejection in `BackupCryptoTest`.
- Dependency refresh wave - updated Gradle, AGP, Kotlin/Compose, KSP, AndroidX, Room, Hilt, OkHttp, DataStore, Browser, SplashScreen, Vico 2.x, Lottie, ZXing, MaxMind, and test dependencies; migrated to AGP 9 built-in Kotlin support; raised compileSdk to 36 while leaving targetSdk at 35; and added `.ai/research/2026-05-17/DEPENDENCY_REFRESH_REPORT.md`.
...
- Argon2id batch verification included `.\app\gradlew.bat -p app --no-parallel :app:compileFullDebugKotlin`, focused `.\app\gradlew.bat -p app --no-parallel :app:testFullDebugUnitTest --tests com.hostshield.data.preferences.SecureStorePinKdfTest --tests com.hostshield.util.BackupCryptoTest`, `.\app\gradlew.bat -p app --no-parallel :app:dependencyInsight --dependency bcprov-jdk18on --configuration fullDebugRuntimeClasspath`, full `.\app\gradlew.bat -p app --no-parallel :app:testFullDebugUnitTest`, `tools/check-release-docs.ps1`, and `git diff --check`.
- AES-GCM nonce guard batch verification included `.\app\gradlew.bat -p app --no-parallel :app:compileFullDebugKotlin`, focused `.\app\gradlew.bat -p app --no-parallel :app:testFullDebugUnitTest --tests com.hostshield.util.BackupCryptoTest`, full `.\app\gradlew.bat -p app --no-parallel :app:testFullDebugUnitTest`, `tools/check-release-docs.ps1`, and `git diff --check`.
- Dependency refresh batch verification included `.\app\gradlew.bat -p app --version`, `.\app\gradlew.bat -p app --no-parallel :app:compileFullDebugKotlin`, full `.\app\gradlew.bat -p app --no-parallel :app:testFullDebugUnitTest`, `.\app\gradlew.bat -p app --no-parallel :app:assembleFullDebug`, `.\app\gradlew.bat -p app --no-parallel :app:assemblePlayDebug`, emulator install `adb install -r app\app\build\outputs\apk\full\debug\app-full-debug.apk`, launch smoke `adb shell am start -W -n com.hostshield.debug/com.hostshield.MainActivity`, `tools\check-release-docs.ps1`, and `git diff --check`.
