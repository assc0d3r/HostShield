...
- [ ] **Room migration golden test matrix.** Create frozen DB fixtures for every supported version path and test v1->v14 upgrades without destructive fallback. Fill missing schema exports or document why older versions cannot be reconstructed. Sources: L024, L025, L026, E010.
...
- [ ] **Hagezi pack chooser.** Add curated Hagezi Light, Multi-Light, Normal, Pro, Pro++, Ultimate, TIF, TIF-Mini, DynDNS, NRD, and Most-Abused-TLDs packs with clear breakage warnings and expected size. Sources: L029, E026.
...
- [ ] **TalkBack semantics pass.** Audit all primary screens and icon-only controls for useful `contentDescription`, `stateDescription`, headings, disabled states, destructive action labels, and progress announcements. Sources: L003, L031, E011.
...
- [ ] **Replace AndroidX Security Crypto alpha dependency.** Design and implement migration from `EncryptedSharedPreferences` / `MasterKeys` to a maintained local secret-store path using Android Keystore and an auditable encryption wrapper. Must migrate existing secrets without data loss. Sources: L022, E009.
