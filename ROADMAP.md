...
- **Magisk 26+ `su --mount-master` audit.** Root firewall path silently no-ops in mount-namespace–sandboxed shells. Detect Magisk version, prefer `su --mount-master` for `iptables`/`ip6tables` invocations. `3 / 1 / Low` `[A5]`
- **`VpnService.Builder.addRoute` host-bit canonicalization.** Pre-canonicalize IPs via `InetAddresses.forString` before adding routes; current code can `IllegalArgumentException` on Android 11+ when a user pastes an address with non-zero host bits. `3 / 1 / Low` `[A6]`

### DNS engine
- **DoH3 (RFC 9484) resolver alongside DoH/DoT/DoQ.** Implement on top of OkHttp 5 / `okhttp-coroutines` HTTP/3 (when stable) or via a thin Quiche-Android binding. Rank in the same latency-EMA ladder. Android 11+ already uses DoH3 transparently for well-known providers via Play System Updates — match that on devices/distros that don't get the module (AOSP, GrapheneOS forks). `4 / 4 / Med` `[A7]`
- **DNSCrypt v2 + Anonymized DNSCrypt relay support.** Parse `sdns://` stamps, support resolver and relay roles, route resolver→relay so upstream never sees client IP. Library-of-record: a Kotlin reimpl of `dnscrypt-proxy`'s plugin chain — start from `plugin_block_name.go` for matching semantics. `3 / 4 / Med` `[A8]`
