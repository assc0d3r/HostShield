...
- `BackupNonceLedger` tracks recent v2 backup key-id/IV tuples, represented by public Argon2id params plus salt plus IV, and refuses export if a tuple repeats in the app process.
...
- `BackupCryptoTest` covers Argon2id backup headers, AES-GCM roundtrip, wrong-passphrase authentication failure, short payload rejection, invalid header/version rejection, legacy PBKDF2 backup import, random salt/IV/ciphertext uniqueness, duplicate key/IV tuple rejection, legacy plaintext detection, and encrypted-import prompt/failure behavior through `BackupRestoreUtil.decodeBackupBytes`.
